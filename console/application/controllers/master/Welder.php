...
class Welder extends CI_Controller
{
	...
	{  
        ...
        $view_data['add_button_url']		=	'master/Welder/addUpdateForm';
    	$view_data['datatable_url']			=	'master/Welder/datatable';
    	$view_data['pdf_url']				=	'master/Welder/downloadPDF';
    	$view_data['pdf_fileName']			=	lang('mm_masters_welder_exportPDFFileName').date('d_m_Y');    	
    	$view_data['export_url']			=	'master/Welder/exportExcel';
    	$view_data['export_fileName']		=	lang('mm_masters_welder_exportPDFFileName').date('d_m_Y');
		...
		$data = array(
                    'title'     	=> lang('mm_masters_welder_manage_pergramable_code_manage_view'),
                    'content'   	=>$this->load->view('master/Welder/WelderDataList',$view_data,TRUE)                
                    );
	...
	function datatable(){
		...
		$fields_arrayPackage = array(
			'j.welder_id','j.welder_name','u.first_name','j.createOn as createOn','u1.first_name as firstname','j.updateOn as updateOn'
		);
		$join_arrayPackage = array(
			'users AS u' => 'u.id = j.createBy',
			'users AS u1' => 'u1.id = j.updateBy',
		);
		$where_arrayPackage = array('j.welder_show_status' =>'1');
		$orderPackage = $sidx.' '. $sord;

		$count =   $this->mcommon->join_records_counts($fields_arrayPackage, 'jr_welder as j', $join_arrayPackage, $where_arrayPackage, '', $orderPackage);
		...
		$dataTable_Details =   $this->mcommon->join_records_all($fields_arrayPackage, 'jr_welder as j', $join_arrayPackage, $where_arrayPackage, '', $orderPackage,'object');
		...
		if (isset($dataTable_Details) && !empty($dataTable_Details)) {
	        ...
	        foreach ($dataTable_Details->result() as $dataDetail) {
	        	...
	        	$responce->rows[$i]['cell']['edit_welder_id'] = get_buttons_new_only_Edit($dataDetail->welder_id,'master/Welder/');
	        	$responce->rows[$i]['cell']['delete_welder_id'] = get_buttons_new_only_Delete($dataDetail->welder_id,'master/Welder/');
	        	$responce->rows[$i]['cell']['welder_name'] = $dataDetail->welder_name;
	...
	{  
        ...
        $view_data['form_url']				=	'master/Welder/create';
        $view_data['form_cancel_url']		=	'master/Welder';
		...
		$data = array(
                    ...
                    'content'   	=>$this->load->view('master/Welder/Weldermanagement',$view_data,TRUE)                
                    );
        $this->load->view('base/template', $data); 
	}
	...
	{		
		...
		{			
			 $this->form_validation->set_rules('welder_name', lang('mm_masters_welder_welder_name'), 'required');
			 
			 $nameVal = $this->input->post('welder_name');

			 $whereArrNameVal        = "welder_name ='".$nameVal."'";

			 // Unique name validation
			 $resultCounteng = $this->mcommon->specific_record_counts('jr_welder',$whereArrNameVal);
			 
			 if($resultCounteng > 0 && $this->input->post('welder_id')==''){
				$this->form_validation->set_rules('welder_name', $this->lang->line('form_validation_is_unique_name'), 'callback_maximumNameCheck'); 
			} 
		...
		if($result)
		{
			...
			$this->session->set_flashdata('res_welder', 'success');
			redirect(base_url().'master/Welder');
		}
		if($resultupdate)
		{
	...
	{   
       ...
       $delete=$this->mcommon->common_delete('jr_welder',$where_array);
		if($delete)
		{
			...
			$this->session->set_flashdata('res_welder', 'danger');
			redirect(base_url().'master/Welder');
		}
		else
		{
