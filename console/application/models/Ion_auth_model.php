...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Ben Edmunds
	 **/
	public function hash_groups_db($identity, $userPermissin)
	{	
		$this->trigger_events('group');
		if (isset($identity))
		{
			//$query = $this->db->select('SELECT t1.email FROM `users` as t1  where t1.email = "'.$identity.'"');
			//echo "SELECT t1.email FROM `users` as t1 Left JOIN users_groups as ug on t1.id = ug.user_id where t1.email = '".$identity."' AND ug.group_id = '".$userPermissin."'";

			/*$query = $this->db->select($this->identity_column . ', emai, id, password, active, last_login,st_storeID')
			->where($this->identity_column, $identity)
			->where($this->identity_column, $identity)
			->get($this->tables['users']);*/


		$query = $this->db->select($this->tables['users_groups'].'.'.$this->join['users'].' as id, '.$this->tables['users'].'.email ')
		                ->where($this->tables['users'].'.'.$this->identity_column, $identity)
		                ->where($this->tables['users_groups'].'.group_id', $userPermissin)
		                ->join($this->tables['users'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.id')
		                ->get($this->tables['users_groups']);

			/*$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,st_storeID')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);*/
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $userPermissin, $remember=FALSE)
	{
		...
		$loginAttempet = $this->hash_groups_db($identity, $userPermissin);
		
		if ($loginAttempet === FALSE)
		{
			$this->set_error('login_unsuccessfulGroup');
			return FALSE;
		}
