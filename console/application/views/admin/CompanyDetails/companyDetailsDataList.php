    ...
    <div class="col-sm-12">
      ...
      <?php
        ...
        <div class="row m-b-10">
          <div class="col-sm-12">  
            <?php 
            if($this->mcommon->getISUserPermission('CompanyDetails add and edit',$this->session->userdata('user_id'))){
              ?>
              <span class="pull-left">
                <a href="<?php echo base_url().$add_button_url;?>" class="btn btn-success waves-effect waves-light m-r-10 pull-right" ><?php echo lang('mm_masters_companydetails_manage_form_add_button_name');?></a>
              </span>
              <?php
            }
          ?>
            <span class="pull-right"> 
              ...
              <!--
                ...
                -->
                  ...
                  ?>
              <?php 
                if($this->mcommon->getISUserPermission('CompanyDetails all details download excel',$this->session->userdata('user_id'))){
                  ?>
                ...
                if($this->mcommon->getISUserPermission('CompanyDetails all details download pdf',$this->session->userdata('user_id'))){
                  ?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      jQuery("#dataTableList2").jqGrid({
        ...
        colModel:[
        <?php if($this->mcommon->getISUserPermission('CompanyDetails add and edit',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_edit_operation');?>' ,name:'edit_company_id',index:'edit_company_id', width:75, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php } if($this->mcommon->getISUserPermission('CompanyDetails delete',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_delete_operation');?>' ,name:'delete_company_id',index:'delete_company_id', width:75, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php }?>
          ...
          {label:'<?php echo lang('mm_operation_company_table_label_company_address');?>',name:'company_address',index:'company_address', width:150, hidden:true, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},

          {label:'<?php echo lang('mm_masters_companydetails_table_label_createBy');?>',name:'first_name',index:'first_name', width:100,  coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_companydetails_table_label_createOn');?>',name:'createOn',index:'createOn', width:150, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},    
          {label:'<?php echo lang('mm_masters_companydetails_table_label_updateBy');?>',name:'firstname',index:'firstname', width:100, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},   
          {label:'<?php echo lang('mm_masters_companydetails_table_label_updateon');?>',name:'updateOn',index:'updateOn', width:150, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}} 
        ],
        rowNum:10,
    ...
    jQuery("#dataTableList2").jqGrid('navButtonAdd','#dataTablePager2',{ caption: "Reorder Columns", buttonicon : "octicon octicon-calendar",title: "Reorder Columns", onClickButton : function (){ jQuery("#dataTableList2").jqGrid('columnChooser') } });
      ...
      $("#pdf").on("click", function(){
        $("#dataTableList2").jqGrid("exportToPdf",{
          title: '<?php echo lang('mm_operation_company_pdfTitle');?>',
