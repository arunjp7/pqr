...
<div id="main-wrapper">
  <div class="text-center">
    ...
    <?php
      ...
      {
        ?>
        <div class="alert alert-<?php echo $this->session->flashdata('res_grade'); ?> successmessage">
  ...
  <?php 
    if($this->mcommon->getISUserPermission('Grade add and edit',$this->session->userdata('user_id'))){
      ...
      <span class="pull-left">
        <a href="<?php echo base_url().$add_button_url;?>" class="btn btn-sm btn-success waves-effect waves-light m-r-10 pull-right" ><?php echo lang('mm_masters_grade_manage_form_add_button_name');?></a>
      </span>
      <?php
    }
  ?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      jQuery("#dataTableList2").jqGrid({
        ...
        <?php if($this->mcommon->getISUserPermission('Grade add and edit',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_edit_operation');?>' ,name:'edit_grade_id',index:'edit_grade_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php } if($this->mcommon->getISUserPermission('Grade delete',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_delete_operation');?>' ,name:'delete_grade_id',index:'delete_grade_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php }?>

           {label:'<?php echo lang('mm_masters_grade_table_label_grade_name');?>',name:'grade_name',index:'grade_name', width:200, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_grade_table_label_createBy');?>',name:'first_name',index:'first_name', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_grade_table_label_createOn');?>',name:'createOn',index:'createOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},    
          {label:'<?php echo lang('mm_masters_grade_table_label_updateBy');?>',name:'firstname',index:'firstname', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},   
          {label:'<?php echo lang('mm_masters_grade_table_label_updateon');?>',name:'updateOn',index:'updateOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}} 
        ],
    ...
    $("#pdf").on("click", function(){
        $("#dataTableList2").jqGrid("exportToPdf",{
          title: '<?php echo lang('mm_masters_grade_pdfTitle');?>',
