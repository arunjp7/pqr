    ...
    <div class="col-sm-12">
      ...
      <?php
        ...
        <div class="row m-b-10">
          <div class="col-sm-12">          
            <?php 
            if($this->mcommon->getISUserPermission('RT Procedure add and edit',$this->session->userdata('user_id'))){
              ?>
              <span class="pull-left">
                <a href="<?php echo base_url().$add_button_url;?>" class="btn btn-success waves-effect waves-light m-r-10 pull-right" ><?php echo lang('mm_masters_RTProcedure_manage_form_add_button_name');?></a>
              </span>
              <?php
            }
          ?>
            <span class="pull-right"> 
              ...
              <!--
                ...
                -->
                  ...
                  ?>
              <?php 
                if($this->mcommon->getISUserPermission('RT Procedure all details download excel',$this->session->userdata('user_id'))){
                  ?>
                ...
                if($this->mcommon->getISUserPermission('RT Procedure all details download pdf',$this->session->userdata('user_id'))){
                  ?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      jQuery("#dataTableList2").jqGrid({
        ...
        colModel:[
        <?php if($this->mcommon->getISUserPermission('RT Procedure add and edit',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_edit_operation');?>' ,name:'edit_rt_procedure_id',index:'edit_rt_procedure_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php } if($this->mcommon->getISUserPermission('RT Procedure delete',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_delete_operation');?>' ,name:'delete_rt_procedure_id',index:'delete_rt_procedure_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php }?>

           {label:'<?php echo lang('mm_masters_RTProcedure_table_label_RTProcedure_no');?>',name:'rt_procedure_name',index:'rt_procedure_name', width:200, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_RTProcedure_table_label_createBy');?>',name:'first_name',index:'first_name', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_RTProcedure_table_label_createOn');?>',name:'createOn',index:'createOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},    
          {label:'<?php echo lang('mm_masters_RTProcedure_table_label_updateBy');?>',name:'firstname',index:'firstname', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},   
          {label:'<?php echo lang('mm_masters_RTProcedure_table_label_updateon');?>',name:'updateOn',index:'updateOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}} 
        ],
    ...
    $("#pdf").on("click", function(){
        $("#dataTableList2").jqGrid("exportToPdf",{
          title: '<?php echo lang('mm_masters_RTProcedure_pdfTitle');?>',
