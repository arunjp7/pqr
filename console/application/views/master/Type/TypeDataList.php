    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            ...
            <div class="panel-body">
              <div class="row">      
  <?php 
    if($this->mcommon->getISUserPermission('Type add and edit',$this->session->userdata('user_id'))){
      ?>
      <span class="pull-left">
        <a href="<?php echo base_url().$add_button_url;?>" class="btn btn-sm btn-success waves-effect waves-light m-r-10 pull-right" ><?php echo lang('mm_masters_type_manage_form_add_button_name');?></a>
      </span>
      <?php
    }
  ?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      jQuery("#dataTableList2").jqGrid({
        ...
        colModel:[
        <?php if($this->mcommon->getISUserPermission('type add and edit',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_edit_operation');?>' ,name:'edit_type_id',index:'edit_type_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php } if($this->mcommon->getISUserPermission('type delete',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_delete_operation');?>' ,name:'delete_type_id',index:'delete_type_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php }?>

           {label:'<?php echo lang('mm_masters_type_table_label_type_name');?>',name:'type_name',index:'type_name', width:200, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_type_table_label_createBy');?>',name:'first_name',index:'first_name', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_type_table_label_createOn');?>',name:'createOn',index:'createOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},    
          {label:'<?php echo lang('mm_masters_type_table_label_updateBy');?>',name:'firstname',index:'firstname', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},   
          {label:'<?php echo lang('mm_masters_type_table_label_updateon');?>',name:'updateOn',index:'updateOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}} 
        ],
    ...
    $("#pdf").on("click", function(){
        $("#dataTableList2").jqGrid("exportToPdf",{
          title: '<?php echo lang('mm_masters_type_pdfTitle');?>',
