  ...
  <div class="text-center">
    <?php
      ...
      {
        ?>
        <div class="alert alert-<?php echo $this->session->flashdata('res_welder'); ?> successmessage">
    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            ...
            <div class="panel-body">
              <div class="row">         
  <?php 
    if($this->mcommon->getISUserPermission('Grade add and edit',$this->session->userdata('user_id'))){
      ?>
      <span class="pull-left">
        <a href="<?php echo base_url().$add_button_url;?>" class="btn btn-sm btn-success waves-effect waves-light m-r-10 pull-right" ><?php echo lang('mm_masters_welder_manage_form_add_button_name');?></a>
      </span>
      <?php
    }
  ?>
  <span class="pull-right"> 
    ...
    <!--
      ...
      -->
        ...
        ?>
    <?php 
      if($this->mcommon->getISUserPermission('Grade all details download excel',$this->session->userdata('user_id'))){
        ?>
      ...
      if($this->mcommon->getISUserPermission('Grade all details download pdf',$this->session->userdata('user_id'))){
        ?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      jQuery("#dataTableList2").jqGrid({
        ...
        colModel:[
        <?php if($this->mcommon->getISUserPermission('Grade add and edit',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_edit_operation');?>' ,name:'edit_welder_id',index:'edit_welder_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php } if($this->mcommon->getISUserPermission('Grade delete',$this->session->userdata('user_id'))){?>
          {label:'<?php echo lang('common_table_label_delete_operation');?>' ,name:'delete_welder_id',index:'delete_welder_id', width:120, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
        <?php }?>

           {label:'<?php echo lang('mm_masters_welder_table_label_welder_name');?>',name:'welder_name',index:'welder_name', width:200, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_welder_table_label_createBy');?>',name:'first_name',index:'first_name', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},
          {label:'<?php echo lang('mm_masters_welder_table_label_createOn');?>',name:'createOn',index:'createOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},    
          {label:'<?php echo lang('mm_masters_welder_table_label_updateBy');?>',name:'firstname',index:'firstname', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}},   
          {label:'<?php echo lang('mm_masters_welder_table_label_updateon');?>',name:'updateOn',index:'updateOn', width:250, sortable:false, coloptions : {sorting:true, columns: true, filtering: true, grouping:true, freeze : true}} 
        ],
    ...
    $("#pdf").on("click", function(){
        $("#dataTableList2").jqGrid("exportToPdf",{
          title: '<?php echo lang('mm_masters_welder_pdfTitle');?>',
