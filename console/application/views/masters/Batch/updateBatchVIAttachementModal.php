...
if(isset($value) && !empty($value))
{
  ...
  {  
    $batchVI_attachement_id               =  $row->batchVI_attachement_id;
    $batchVI_attachement_type_name        =  $row->batchVI_attachement_type_name;
    $batchVI_attachement_file_name        =  $row->batchVI_attachement_file_name;   
    $batchVI_old_attachement_file_name    =  $row->batchVI_attachement_file_name;   
  }
}
else
{
  $batchVI_attachement_id                  =  $this->input->post('batchVI_attachement_id');
  $batchVI_attachement_type_name           =  $this->input->post('batchVI_attachement_type_name');
  $batchVI_attachement_file_name           =  $this->input->post('batchVI_attachement_file_name');
  $batchVI_old_attachement_file_name       =  $this->input->post('batchVI_old_attachement_file_name');
}
$ci =&get_instance();
  ...
  <form ng-app="form-example" method="post" id="ajaxModelForm" enctype="multipart/form-data" action="javascript:" ui-jp="parsley">
    <div class="modal-body">
        ...
        <div class="col-sm-4 col-xs-4">
          ...
          <div class="row">
            <div class="col-sm-12 col-xs-12">
              <div class="form-group <?PHP if(form_error('batchVI_attachement_type_name')){ echo 'has-error';} ?>">
                <label for="exampleInputEmail1"><?php echo lang('mm_masters_batch_Modal_attachement_type_name');?></label>
                <input type="text" class="form-control" placeholder="<?php echo lang('mm_masters_batch_Modal_attachement_type_name');?>" id="batchVI_attachement_type_name" name="batchVI_attachement_type_name" value="<?php echo $batchVI_attachement_type_name ;?>" autocomplete="off">
               <?PHP if(form_error('batchVI_attachement_type_name')){ echo '<span class="help-block">'.form_error('batchVI_attachement_type_name').'</span>';} ?>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-sm-12 col-xs-12">
              <div class="form-group <?PHP if(form_error('batchVI_attachement_file_name')){ echo 'has-error';} ?>">
              <label for="exampleInputEmail1"><?php echo lang('mm_masters_batch_Modal_attachement_file_name');?> <?php echo lang('mm_common_logo_size_label_500');?> </label>
                <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                  ...
                  <input type="file" id="batchVI_attachement_file_name" name="batchVI_attachement_file_name" >
                ...
                <?PHP if(form_error('batchVI_attachement_file_name')){ echo '<span class="help-block">'.form_error('batchVI_attachement_file_name').'</span>';} ?>
              </div>
            </div>
      ...
      <div class="text-center">
        ...
        <input type="hidden" name="batchVI_attachement_id" value="<?php echo $batchVI_attachement_id;?>">
        <input type="hidden" name="batchVI_old_attachement_file_name" value="<?php echo $batchVI_old_attachement_file_name;?>" />
              ...
              try 
                {
                  ...
                  if(parsedJson.result == 'Success' || parsedJson.result == "Update"  || parsedJson.result == "ExistRecord")
                  {
                    ...
                    $('#batchVI_attachement_type_name').val('');
                    $('#batchVI_attachement_file_name').val('');
